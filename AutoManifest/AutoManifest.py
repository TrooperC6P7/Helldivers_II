...
def main():
    ...
    for org_dir in sorted(main_dir.iterdir()):
        #make sure it's a folder
        if org_dir.is_dir():
            #loop through option folders
            for mod_sub_dir in sorted(org_dir.iterdir()):
                #make sure it's a folder
                if mod_sub_dir.is_dir():
                    ...
                    if any(item.is_dir() for item in mod_sub_dir.iterdir()):
                        ...
                        for mod_subsub_dir in sorted(mod_sub_dir.iterdir()):
                            #make sure it's a folder
                            if mod_subsub_dir.is_dir():
                                ...
                                if any(item.is_dir() for item in mod_subsub_dir.iterdir()):
                                    input(f"Invalid folder nesting: {mod_subsub_dir}! Press any key to exit...")
                                    sys.exit()
                                
                                #skip if sub-option folder doesn't have patch files
                                elif not any(".patch_" in str(item.name) for item in mod_subsub_dir.iterdir()):
                                    continue

                                #create new sub-option
                                sub_option = LeafOption()
                                sub_option.Name = mod_subsub_dir.name
                                sub_option.Include = [f"{org_dir.name}/{mod_sub_dir.name}/{mod_subsub_dir.name}"]
                                ...
                                if prev_manifest != None:
                                    sub_option.Description = next((
                                        ...
                                        and sub_item.get("Name") == mod_subsub_dir.name), "")

                                #find and add sub-option image
                                images = sorted(main_dir.rglob(f"{mod_sub_dir.stem} {mod_subsub_dir.stem}.[jp][pn]g"))
                    ...
                    elif any(".patch_" in str(item.name) for item in mod_sub_dir.iterdir()):
                        #create new option
                        option = LeafOption()
                    ...
                    option.Include = [f"{org_dir.name}/{mod_sub_dir.name}"]
